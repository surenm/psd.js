...
arraySum = function(arr, from, to) {
  ...
  END DEPENDENCIES
  */
  var Log, PSD, PSDDropDownLayerEffect, PSDFile, PSDHeader, PSDImage, PSDLayer, PSDLayerEffect, PSDLayerMask, PSDResource, Root, Util, fs,
  ...
  Root.PSD = PSD = (function() {
    ...
    PSD.prototype.parseImageData = function() {
      var compression, length;
      if (!this.header) this.parseHeader();
      compression = this.file.readShortInt();
      length = this.file.data.length - this.file.tell();
      Log.debug("" + length + " bytes until EOF. Parsing image data...");
      this.image = new PSDImage(this.file, compression, this.header, length);
  ...
  PSDImage = (function() {
    ...
    function PSDImage(file, compression, header, length) {
      this.file = file;
    ...
    PSDImage.prototype.parseRaw = function() {
      var color, i, _i, _len, _ref, _results;
      Log.debug("Attempting to parse RAW encoded image...");
      this.size = this.width * this.height;
      Log.debug("Image size: " + this.size + " (" + this.width + "x" + this.height + ")");
      _ref = ['r', 'g', 'b'];
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        _results.push((function() {
          var _ref2, _results2;
          _results2 = [];
          for (i = 0, _ref2 = this.size; 0 <= _ref2 ? i < _ref2 : i > _ref2; 0 <= _ref2 ? i++ : i--) {
            _results2.push(this.pixelData[color].push(this.file.read(2)[0]));
          }
          return _results2;
        }).call(this));
      }
    ...
    PSDImage.prototype.parseRLE = function() {
      return Log.debug("Attempting to parse RLE encoded image...");
    };
    ...
    PSDImage.prototype.toCanvasPixels = function() {
      ...
      for (i = 0, _ref = this.pixelData.r.length; 0 <= _ref ? i < _ref : i > _ref; 0 <= _ref ? i++ : i--) {
        result.push(this.pixelData.r[i], this.pixelData.g[i], this.pixelData.b[i], 255);
      }
      return result;
    };
  ...
  PSDLayer = (function() {
    ...
    PSDLayer.prototype.parse = function(layerIndex) {
      ...
      this.name = this.file.readString(namelen)[0];
    ...
    PSDLayer.prototype.parseExtraData = function() {
      ...
      while (this.file.tell() < this.layerEnd) {
        ...
        switch (key) {
          case "lyid":
            _results.push(this.layerId = this.file.readUInt());
            break;
          case "shmd":
            _results.push(this.readMetada());
            break;
          case "lsct":
            _results.push(this.readLayerSectionDivider());
            break;
          case "luni":
            _results.push(this.uniName = this.file.readUnicodeString());
            break;
          case "vmsk":
            _results.push(this.readVectorMask());
            break;
          case "TySh":
            _results.push(this.readTypeTool());
            break;
          case "lrFX":
            _results.push(this.parseEffectsLayer());
            break;
          default:
            this.file.seek(length);
            _results.push(Log.debug("Skipping additional layer info with key " + key));
        }
      }
      return _results;
    };
    ...
    PSDLayer.prototype.parseEffectsLayer = function() {
      ...
      while (count-- > 0) {
        ...
        if (effect != null) {
          ...
          size = this.file.readShortUInt();
          _results.push(this.file.seek(size));
        }
      }
      return _results;
    };
    ...
    PSDLayer.prototype.isFolder = function() {
      return this.type === SECTION_DIVIDER_TYPES[1] || this.type === SECTION_DIVIDER_TYPES[2];
    };
  ...
  PSDLayerMask = (function() {
    ...
    PSDLayerMask.prototype.parse = function() {
      var endLoc, i, layer, layerInfoSize, maskSize, pos, _ref;
  ...
  PSDLayerEffect = (function() {
    ...
    PSDLayerEffect.prototype.parse = function() {
      var _ref;
      return _ref = this.file.readf(">ii"), this.size = _ref[0], this.version = _ref[1], _ref;
    };
  ...
  PSDDropDownLayerEffect = (function(_super) {
    ...
    PSDDropDownLayerEffect.prototype.parse = function() {
      var alpha, angle, enabled, useInAllEFX, _ref, _ref2;
      PSDDropDownLayerEffect.__super__.parse.call(this);
      _ref = this.file.readf(">hiii"), this.blur = _ref[0], this.intensity = _ref[1], angle = _ref[2], this.distance = _ref[3];
      this.angle = 180 - angle;
      this.file.read(4);
      this.color = this.file.readf(">BBBB");
      _ref2 = this.file.readf(">4s4s"), this.signature = _ref2[0], this.blendMode = _ref2[1];
      enabled = this.file.readBoolean();
      useInAllEFX = this.file.readBoolean();
      alpha = this.file.read(1)[0] / 255.0;
      this.file.read(4);
      return this.nativeColor = this.file.readf(">BBBB");
    };
