  ...
  */
  ...
  PSDImage = (function() {
    ...
    PSDImage.prototype.parseRLE = function() {
      var byteCounts, i, j, len, lineIndex, pos, src, x, _ref, _ref2, _ref3, _ref4, _ref5;
      ...
      Log.debug("Read byte counts. Current pos = " + (this.file.tell()) + ", End = " + this.endPos);
      this.channelData = [];
      for (x = 0, _ref3 = this.length; 0 <= _ref3 ? x < _ref3 : x > _ref3; 0 <= _ref3 ? x++ : x--) {
        this.channelData.push(0);
      }
      for (i = 0, _ref4 = this.header.channels; 0 <= _ref4 ? i < _ref4 : i > _ref4; 0 <= _ref4 ? i++ : i--) {
        lineIndex = 0;
        pos = 0;
        for (j = 0, _ref5 = this.height; 0 <= _ref5 ? j < _ref5 : j > _ref5; 0 <= _ref5 ? j++ : j--) {
          len = byteCounts[lineIndex++];
          src = this.file.read(len);
          this.decodeRLE(src, 0, len, this.channelData, pos);
          pos += this.width;
        }
      }
    ...
    PSDImage.prototype.decodeRLE = function(src, sindex, slen, dst, dindex) {
    ...
    PSDImage.prototype.combineRGB8Channel = function() {
      ...
      for (i = 0, _ref = this.channelLength; 0 <= _ref ? i < _ref : i > _ref; 0 <= _ref ? i++ : i--) {
        if (this.header.channels === 4) this.pixelData.a[i] = this.channelData[i];
        this.pixelData.r[i] = this.channelData[i + this.channelLength];
        this.pixelData.g[i] = this.channelData[i + (this.channelLength * 2)];
        _results.push(this.pixelData.b[i] = this.channelData[i + (this.channelLength * 3)]);
      }
      return _results;
    };
